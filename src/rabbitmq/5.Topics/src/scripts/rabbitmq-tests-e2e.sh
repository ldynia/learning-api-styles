...
if [[ "$(basename $(pwd))" != "5.Topics" ]];
then
    echo "Error: this script must be run from the '5.Topics' directory"
    exit 1
fi
...
docker compose exec consumer-a python3 /usr/src/consumer.py "*.critical"
...
docker compose exec consumer-b python3 /usr/src/consumer.py "kernel.*"
# end::start_consumer-b[]
unset docker
sleep 1

screen -ls || true
LOGFILE_CONSUMER_C=src/scripts/e2e-tests-consumer-c.log
echo > $LOGFILE_CONSUMER_C
docker compose exec consumer-c bash -c 'kill $(pgrep --full "python3 /usr/src/consumer.py")' || true
function docker () {
    screen -L -Logfile $LOGFILE_CONSUMER_C -S consumer-c -d -m docker "$@"
}
# tag::start_consumer-c[]
docker compose exec consumer-c python3 /usr/src/consumer.py "#"
# end::start_consumer-c[]
unset docker
sleep 1
...
screen -ls || true
echo "quit consumer-c" && screen -X -S "consumer-c" quit
screen -ls || true
...
echo "cat $LOGFILE_CONSUMER_C" && cat $LOGFILE_CONSUMER_C && echo

# Test $MESSAGES_PRODUCER, $LOGFILE_CONSUMER_A and $LOGFILE_CONSUMER_B contains more than 12 messages
...
MESSAGES_CONSUMER_C=$((grep 'Received .* message' $LOGFILE_CONSUMER_C || true) | wc -l | xargs)
if [ $(($MESSAGES_PRODUCER + $MESSAGES_CONSUMER_A + $MESSAGES_CONSUMER_B + $MESSAGES_CONSUMER_C)) -gt 12 ]; then
...
# rm $LOGFILE_PRODUCER $LOGFILE_CONSUMER_A $LOGFILE_CONSUMER_B $LOGFILE_CONSUMER_C