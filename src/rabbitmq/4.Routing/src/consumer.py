...
class RabbitMQConsumer:
    ...
    def __init__(self):
        if not self.routing_keys:
            self.routing_keys = [random.choice(("info", "warning", "error"))]
        self.__connect()
    ...
    def __bind_queue(self):
        for severity in self.routing_keys:
    ...
    def __callback(self, channel, method, properties, msg):
        ...
        print(f"[x] Received '{msg}' message from '{self.exchange}' exchange.")

    def consume(self):
        print(f"[*] Waiting for messages from '{self.exchange}' exchange on '{', '.join(self.routing_keys)}' routing_key. To exit press CTRL+C")
