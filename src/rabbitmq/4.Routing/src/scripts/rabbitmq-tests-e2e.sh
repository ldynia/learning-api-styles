...
if [[ "$(basename $(pwd))" != "4.Routing" ]];
then
    echo "Error: this script must be run from the '4.Routing' directory"
    exit 1
fi
...
docker compose exec consumer-a python3 /usr/src/consumer.py info
...
docker compose exec consumer-b python3 /usr/src/consumer.py error warning
...
# Test $MESSAGES_PRODUCER, $LOGFILE_CONSUMER_A and $LOGFILE_CONSUMER_B contains more than 6 messages
MESSAGES_PRODUCER=$((grep 'Sent .* message' $LOGFILE_PRODUCER || true) | wc -l | xargs)
MESSAGES_CONSUMER_A=$((grep 'Received .* from' $LOGFILE_CONSUMER_A || true) | wc -l | xargs)
MESSAGES_CONSUMER_B=$((grep 'Received .* from' $LOGFILE_CONSUMER_B || true) | wc -l | xargs)
if [ $(($MESSAGES_PRODUCER + $MESSAGES_CONSUMER_A + $MESSAGES_CONSUMER_B)) -gt 6 ]; then
